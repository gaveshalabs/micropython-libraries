...
# Author Lihini Senanayake for Gavesha Labs Pvt Ltd
...
def init_bmp280(scl_pin=None, sda_pin=None, freq=400000, addr=0x76, use_case=BMP280_CASE_HANDHELD_DYN) :
    if not isinstance(scl_pin, Pin):
        raise RuntimeError("scl_pin should be a Pin()")
    if not isinstance(sda_pin, Pin):
        raise RuntimeError("sda_pin should be a Pin()")
    i2c = I2C(scl = scl_pin, sda = sda_pin, freq=freq)
    return BMP280(i2c_bus=i2c, addr=addr, use_case=use_case)
