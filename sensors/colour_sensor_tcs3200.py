...
class TCS3200(object):
    ...
    def __init__(self, OUT=19, S2=5, S3=18, S0=None, S1=None, LED=None,OE=None):
        ...
        self._cycle = 0
        self._freq_div = self.TWO_PERCENT
        self._start_tick = 0
        self._end_tick = 0
        meas_finished = False
    ...
    def calibrate_black(self, freqs, freq1=0, freq2=0, freq3=0):
        if isinstance(freqs, list):
            self._freq_black = freqs
        else:
            self._freq_black = [freqs, freq1, freq2, freq3]
        
    def calibrate_white(self, freqs, freq1=0, freq2=0, freq3=0):
        if isinstance(freqs, list):
            self._freq_white = freqs
        else:
            self._freq_white = [freqs, freq1, freq2, freq3]
    ...
    def rgb(self):
        ...
        for i in range(4):
            ...
            if argb[i] < 0:
                argb[i] = 0
            elif argb[i] > this_comp_max :
                this_comp_max = argb[i]
        if this_comp_max > self._max_comp:
            argb_final = [int(x/this_comp_max*self._max_comp) for x in argb]
        else:
            argb_final = argb
        if self._debug:
            print("rgb array:",argb_final)
        return argb_final
    
    @property
    def red(self):
        return self._read_rgb_comp(0)
    
    @property
    def green(self):
        return self._read_rgb_comp(1)
    
    @property
    def blue(self):
        return self._read_rgb_comp(2)
    ...
    def _read_rgb_comp(self, comp):
        if not self._freq_black[0] or not  self._freq_white[0]:
            print("Missing calibration. Please calibrate the device before attempting to measure colored targets")
            return
        filter_settings = (self.RED,self.GREEN,self.BLUE,self.CLEAR)
        self.filter = filter_settings[comp]
        self.meas = self.ON       # start the measurement
        while self._end_tick == 0:
            time.sleep_ms(10)
        value = int(self._max_comp*self.calc_rgb_comp(comp, self.measured_freq))
        if value < 0:
            value = 0
        elif value > self._max_comp:
            return self._max_comp
        return value
